...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json({ message: "sharedPrompt comment like toggled" }, { status: 200 });
  } catch (err) {
    return NextResponse.json({ message: "err toggle comment like on sharedPrompt" }, { status: 500 });
  }
}